...
2. [데모 영상](#2-데모-영상)
3. [자동 생성 보고서 샘플 (PDF)](#3-자동-생성-보고서-샘플-pdf)
4. [주요 기능](#4-주요-기능)
5. [실행 방법](#5-실행-방법)
...
12. [세그멘테이션 레이블](#12-세그멘테이션-레이블-뷰어-색상-범례)
...
> 다운로드 중 TLS 오류가 발생하면 네트워크 문제입니다. 
> 잠시 후 재시도하거나 다른 네트워크(핫스팟 등)로 전환 후 다시 실행하세요.
...
최초 실행 시 백엔드 이미지를 빌드합니다 (약 3~5분). 
ChromaDB에 WHO CNS 2021 문서가 자동으로 초기화되므로 첫 보고서 생성은 평소보다 느릴 수 있습니다.
...
백엔드가 DB 연결 전에 먼저 뜨는 경우입니다. `docker compose logs backend`로 오류를 확인하세요. 
DB `healthcheck`가 통과되면 자동으로 정상화됩니다. 수 초 내로 안정되지 않으면 다음을 시도하세요.

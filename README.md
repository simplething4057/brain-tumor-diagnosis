...
../LLM/brain-tumor-3class/   ML 파이프라인 (볼륨 마운트)
```
...
```bash
git clone https://github.com/simplething4057/brain-tumor-3class.git ../LLM/brain-tumor-3class
```

`meta_classifier.pkl`이 없으면 재학습 필요:
...
ML_PIPELINE_PATH=/ml_pipeline    # ../LLM/brain-tumor-3class 마운트 경로
...
ML 파이프라인 폴더가 없거나 모델이 학습되지 않은 상태입니다.

```bash
# 1. 폴더 클론
git clone https://github.com/simplething4057/brain-tumor-3class.git ../LLM/brain-tumor-3class

# 2. 재학습 (features.csv는 GitHub에서 자동 다운로드)
pip install scikit-learn pandas requests
python retrain_classifier.py
